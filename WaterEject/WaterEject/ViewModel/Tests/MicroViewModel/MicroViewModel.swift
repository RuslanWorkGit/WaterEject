...
final class MicroViewModel: NSObject, ObservableObject, AVAudioPlayerDelegate {
    ...
    func stopRecording(save: Bool = true) {
        ...
        Task {
            ...
            let rec = Recording(url: url, duration: duration, createdAt: Date(), title: url.deletingPathExtension().lastPathComponent)
            await MainActor.run {
                recordings.insert(rec, at: 0) // новий згори
                canContinue = true
            }
        }
    }
    ...
    private func push(_ v: Float) {
        ...
        if liveSamples.count >= 60 { _ = liveSamples.removeFirst() }
        liveSamples.append(clamped)
    }
