...
struct OnboardingNewFourthViewOne: View {
    let index: Int
    let action: () -> Void
...
struct OnboardFourthForm<Content: View>: View {
    let ctaTitle: String
    let ctaAction: () -> Void
    var pages: Int = 0
    var pageIndex: Int = 0
        ...
        ZStack { content() }
            .safeAreaInset(edge: .bottom) {
                VStack {
                    ...
                    if pages > 0 {
                        PageDotsFourth(total: pages, index: pageIndex)
//                            .padding(.bottom, 32)
                            .padding(.bottom, 8)
                            //.padding(.top, 12)
                    }
                    ...
                    HStack { // гарантує однакову геометрію
                        ...
                        OboardFourthNewButton(title: ctaTitle, action: ctaAction, arrow: true)
...
struct OboardFourthNewButton: View {
    let title: String
    let action: () -> Void
...
#Preview {
    OnboardingNewFourthViewOne(index: 0) {
        print("1")
    }
}
